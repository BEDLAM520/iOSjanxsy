...
- (void)newTaskWithLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest cache:(BOOL)cache {
    ...
    NSLog(@"pa --  %@  %d  %d  %d",self.requestTask.requestURL, self.requestTask.requestOffset, self.requestTask.cache, self.requestTask.fileLength);
...
//    NSLog(@"---___ %lu",(unsigned long)_requestTask.fileLength);
    //读文件，填充数据
...
//    NSLog(@"respondData     %d      %d      %d    %d",requestedOffset - self.requestTask.requestOffset,requestedOffset, self.requestTask.requestOffset, respondLength);
