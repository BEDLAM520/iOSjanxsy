...
- (void)setPlayerView:(UIView *)playerView {
    ...
    AVPlayerLayer *avlayer = [[AVPlayerLayer alloc]initWithLayer:self.player];
    NSLog(@"--  %@  %@",self.player, avlayer);
    avlayer.videoGravity = AVLayerVideoGravityResizeAspectFill;
    avlayer.contentsScale = [[UIScreen mainScreen] scale] * 0.5;
    avlayer.frame = self.playerView.bounds;
    [playerView.layer addSublayer:avlayer];
    NSLog(@"%@   %@   %@  %@",self.avlayer, _playerView, avlayer, self.player);
}
